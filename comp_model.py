...
def comp_model(t, y, v2, ssri_molecular_weight, SSRI_start_time, SSRI_repeat_time, SSRI_q_inj, 
  ...
  dy[8] = a16 * fireht(y[56], inhibR5HTto5HT(y[11], gstar_5ht_basal) * inhibRHAto5HT(y[15], gstar_ha_basal)) * y[
    6] - VSERT(y[8], y[19], ssri, allo_ssri_ki(ssri)) - a11 * H1ht(y[8], eht_basal) * VUP2(y[8]) - a14 * y[8] + a8 * (
                    y[5] - y[8]) + a9 * (y[14] - y[8])
  ...
  rate = 2*(1/10)  # firing rate (ms-1).
  #weight of differennt channel conductance
  w_NMDA = 0
  w_GABA_A = 1
  w_AMPA = 8
  w_GABA_B = 1

  #conductance of channels,generating noise
  ...
  #whether there is external stimulation or not
  spike = spike_boolean(y[51])
  #weights for the spike function
  w1 = 0.5
  w2 = 0.001
  w3 = 0.4  # 0.08 0.4 1.2
  w4 = 0.05  # 0.1
  ...
  dy[51] = (- I_Na(y[52], y[53], y[51]) - I_K(y[54], y[51]) - I_L(
    y[51]) + w3 * g_AMPA + w4 * g_NMDA + w5 * g_GABA_A + w6 * g_GABA_B + I_ext(t)) / C_m
