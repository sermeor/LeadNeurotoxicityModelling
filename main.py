...
#Initial conditions
y0 = [
  95.9766, 0.0994, 0.9006, 20.1618, 1.6094, 0.0373, 63.0383, 280.0048, 0.0603,
  1.6824, 113.4099, 0.8660, 1.0112, 0.9791, 0.0027, 0.7114, 1.3245, 0.9874,
  0.2666, 1.0203, 0.2297, 0, 0, 0, 0, 0, 3.1074, 136.3639, 241.9217, 1.4378,
  2.0126, 99.7316, 249.3265, 311.6581, 0.7114, 1.3245, 0.9874, 0.8660, 1.0112,
  0.9791, 354.6656, 177.3328, 350, 150, 3, 140, 0, 0, 0, 0, 1, -60, 0.05, 0.6,
  0.32,0, 0, 0, 0, 0, 0, 0
]

arguments = (v2, ssri_molecular_weight, SSRI_start_time, SSRI_repeat_time,
             SSRI_dose * SSRI_bioavailability, fmh_molecular_weight,
             FMH_start_time, FMH_repeat_time, FMH_dose * FMH_bioavailability,
             mc_switch, mc_start_time, btrp0, eht_basal, gstar_5ht_basal,
             gstar_ha_basal, bht0, vht_basal, vha_basal)
...
plt.figure()
plt.plot(time_array * 3600, sol.y[57])
plt.ylabel('Activity')
plt.xlabel('Time (s)')
...
plt.xlabel('Time (s)')
plt.plot(time_array * 3600, sol.y[58], color='coral')  #AMPA
plt.plot(time_array * 3600, sol.y[59], color='indigo')  #GABA A
plt.plot(time_array * 3600, sol.y[60], color='gold')  #NMDA
plt.plot(time_array * 3600, sol.y[61], color='skyblue')  #GABA B
