...
def w_fast_update(wfe, wfe_decay, wfi, wfi_decay, Neurot, w_fast, N, NE):
  alpha_w = np.empty(N, dtype=np.float64)
...
def comp_model(t, y, N, NE):
  ...
  a3 = (1.5)*0.25 #Weight of excitatory NMDA inputs.
  a4 = (1.5)*0.25 #Weight of inhibitory GABA B inputs.
  ...
  dy[6] = w_fast_update(a7, a8, a9, a10, y[5], y[6], N, NE)
  dy[7] =  w_slow_update(a11, a12, a13, a14, y[5], y[7], N, NE)
...
time_array = np.linspace(0, time, math.floor(time * sampling_rate + 1), dtype=np.float64)
...
y0 = np.concatenate((Vm, m, h, n, Ca_0, N_0, g_fast_0, g_slow_0, act, C), axis = 0, dtype=np.float64) #Flatten initial conditions.
arguments = (N, NE) #Constant arguments.

#Get solution of the differential equation.
sol = solve_ivp(comp_model, t_span = (time_array[0], time_array[-1]), t_eval = time_array, y0 = y0, method = 'RK45', args = arguments, rtol = 1e-3, atol = 1e-6)
